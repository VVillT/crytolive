...
st.markdown("""
This app retrieves cryptocurrency prices for the top 20 cryptocurrency from the **CoinMarketCap**!
""")
#---------------------------------#
# About
...
* **Data source:** [CoinMarketCap](https://coinmarketcap.com/all/views/all/).
* **Credit:** Hon Wing Lee@OMD & Web scraper adapted from the Medium article *[Web Scraping Crypto Prices With Python](https://towardsdatascience.com/web-scraping-crypto-prices-with-python-41072ea5b5bf)* written by [Bryan Feng](https://medium.com/@bryanf).
""")
...
def load_data():
    ...
    table = []
    #h7vnx2-2 czTsgW cmc-table' - landing page
    # -all page
    for li in soup.find_all("div", {'class':'cmc-table__table-wrapper-outer'}):
    ...
    df = pd.DataFrame([sub.split("|") for sub in table[3:23]] , columns =['Rank','void6', 'Name', 'Symbol','Cap in text', 'Market Cap', 'Price', 'Circulating Supply','void1', 'Volume(24h)' , '% 1h','void2', '% 24h', 'void3', '% 7d', 'void4'])
    #df.columns = ['Rank', 'Name', 'Symbol', 'Market Cap', 'Price', 'Circulating Supply', 'Volume(24h)' , '% 1h', '% 24h', '% 7d']
    cols = [c for c in df.columns if c.lower()[:4] != 'void']
    output = df[cols]
#df.drop(columns=['8','11','13','15'])
    return output
